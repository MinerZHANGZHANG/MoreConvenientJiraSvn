...
public partial class Jira2LocalDirViewModel(JiraService jiraService, SvnService svnService, IRepository repository, SettingService settingService) : ObservableObject
{
    ...
    public async Task InitViewModelAsync()
    {
        ...
        LocalJiraSetting = _settingService.FindSetting<LocalJiraSetting>() ?? new();
    }
    ...
    public async Task QueryJiraInfo()
    {
        ...
        switch (SelectedJiraQueryType)
        {
            ...
            case JiraQueryType.Filter:
                ...
                var jiraInfos = await _jiraService.GetIssuesDiffByFilterAsync(SelectedFilter);
    ...
    public async void RefreshLocalJiraInfo()
    {
        ...
        SelectedJiraLocalInfo = _repository.FindOne<LocalJiraInfo>(BsonExpression.Create($"IssueKey = \"{SelectedJiraInfo.IssueKey}\""));

        RelatSvnPaths = _jiraService.GetRelatSvnPath(SelectedJiraInfo).ToList();
        SelectedSvnPath = RelatSvnPaths.FirstOrDefault();

        //Transitions = new(await _jiraService.get(SelectedJiraInfo.IssueKey));
    ...
    {
        string? url = $"{_jiraService.JiraConfig.BaseUrl}browse/{SelectedJiraInfo?.IssueKey}";
    ...
    public async Task UseSelectedToCreateLocalJiraAsync()
    {
        ...
        if (Directory.Exists(LocalJiraSetting.ParentDir))
        {
            ...
            if (Directory.Exists(fullDirName))
            {
                ...
                _repository.Upsert<LocalJiraInfo>(SelectedJiraLocalInfo);
                return;
            }
    ...
    public void SetLocalDirForJiraInfo()
    {
        ...
        if (result == true)
        {
            ...
            if (!string.IsNullOrEmpty(selectedPath))
            {
                ...
                _settingService.UpsertSetting(LocalJiraSetting);
    ...
    public async Task DownloadPrevSvnLogByDate(object parameter)
    {
        ...
        await Task.Run(() =>
        {
            ...
            {
                _svnService.GetSvnLogsAsync(SelectedSvnPath.Path,
